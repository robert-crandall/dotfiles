...
1. Decide: agree with the comment and fix, or push back with reasoning, or leave unresolved.
- NOTE: Reviewers can be wrong or misinformed. If you disagree, reply to the thread explaining why. Reviewers can also be nitpicky; do not resolve nitpicks. Leave these unresolved.
1. When you have one or more fixes ready, commit them with a meaningful message and push. Do NOT use emdash (` — `) in this message. 
...
### Step 3: Reply to and resolve each addressed thread

For every thread you addressed, post a reply explaining the fix (referencing the commit SHA), then resolve the thread. Do NOT use emdash (` — `) in this reply. Threads that you did **not** address (or where you disagreed) should get a reply explaining why; resolve only after you and the reviewer agree. Do not do this for nitpick threads — leave those unresolved.
...
### Step 4: Request (or re-request) Copilot Code Review
...
### Step 5: Watch CI and wait for the new CCR review
...
Re-snapshot (Step 1). If there are unresolved threads or failing checks, go back to Step 2. Repeat until **all** of these hold:

- 0 unresolved review threads
- `mergeStateStatus` is `CLEAN` (or `BLOCKED` only on policy items the user is choosing to bypass)
...
All prose this skill produces - status snapshots, commit messages, reply comments on review threads, and summaries back to the user - must follow this style:
...
- No em dashes (—). Use a plain dash ( - ) instead. This matters especially in commit messages and any text posted to GitHub (reply comments, review responses, PR edits).
...
- Wait silently for hours — surface partial progress rather than blocking the conversation.
